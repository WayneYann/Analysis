...
for i in range(0,num_Files):
    ...
    ASE_CONTCAR = ASE_CONTCAR.repeat((2,2,1))
    numAtoms = len(ASE_CONTCAR)
    Coordinates = ASE_CONTCAR.positions
    Oatom1 = ASE_CONTCAR.positions[262,:]
    Oatom2 = ASE_CONTCAR.positions[197,:]
    Oatom1 = ASE_CONTCAR.positions[64,:]
    Oatom2 = ASE_CONTCAR.positions[65,:]
    distance = sum((Oatom1-Oatom2)**2)**(0.5)
...
plt.title('v(O-O) Frequency for O$_{2}$ on Rh(111) PBE-D3',size=14, fontweight='bold')
...
we = frequency
wexe = h/(2*np.pi)*(a*10**(10))**2/(c*4*np.pi*mu)*10**(-2)
print('wexe')
print(wexe)
print('Do')
