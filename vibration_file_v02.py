...
p = np.polyfit(Data['v(M-O)'][idx1],Data['v(M-H)'][idx1], 1) 
m1 = p[0]
b1=p[1]
idx2 = np.isfinite(Data['v(M-O)']) & np.isfinite(Data['v(M-N)'])
p = np.polyfit(Data['v(M-O)'][idx2],Data['v(M-N)'][idx2], 1) 
m2 = p[0]
b2=p[1]
idx3 = np.isfinite(Data['v(M-O)']) & np.isfinite(Data['parallel (M-H)'])
p = np.polyfit(Data['v(M-O)'][idx3],Data['parallel (M-H)'][idx3], 1) 
m3 = p[0]
b3=p[1]
...
plt.plot(np.array([np.min(Data['v(M-O)'][idx1]),np.max(Data['v(M-O)'][idx1])]), m1*np.array([np.min(Data['v(M-O)'][idx1]),np.max(Data['v(M-O)'][idx1])])+b1,'-r')
plt.plot(np.array([np.min(Data['v(M-O)'][idx1]),np.max(Data['v(M-O)'][idx1])]), m3*np.array([np.min(Data['v(M-O)'][idx1]),np.max(Data['v(M-O)'][idx1])])+b3,'-b')
plt.plot(np.array([np.min(Data['v(M-O)'][idx2]),np.max(Data['v(M-O)'][idx2])]), m2*np.array([np.min(Data['v(M-O)'][idx2]),np.max(Data['v(M-O)'][idx2])])+b2,'-g')
plt.xlabel(r'$\mathbf{\nu}_{\perp,O}$ [$cm^{-1}$]',size=32)
...
vibration_file = os.path.expanduser('~/Box Sync/Synced_Files/Coding/Research/Analysis/Energy_Frequency_Slopes_v02.csv')
Data = pd.read_csv(vibration_file,sep=',',header=0)
cols = Data.columns
cols = cols.map(lambda x: x.replace(' ', '_') if isinstance(x, (str, unicode)) else x)
Data.columns = cols
